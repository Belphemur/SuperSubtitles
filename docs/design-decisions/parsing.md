...
**Implementation**: `SubtitleParser` in `internal/parser/subtitle_parser.go` includes `convertLanguageToISO` (Hungarian → ISO 639-1), `parseReleaseInfo` (quality and release groups), `parseDescription` (season/episode/show name), and `detectQuality` (quality enum). All normalization happens during HTML parsing in one pass.
