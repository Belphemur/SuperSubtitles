...
description: "Record an architectural design decision in docs/design-decisions/. Use when a code change involves a non-trivial design choice, trade-off, or new pattern that future contributors should understand."
...
2. **Identify the domain file** — decisions are grouped by domain (e.g., `cache.md`, `streaming.md`, `parsing.md`). Pick the existing file that best fits. If no file matches, create a new one named after the domain.
...
- Don't duplicate information already in `docs/data-flow.md` or `docs/grpc-api.md` — those describe behaviour, design decisions explain _why_.
