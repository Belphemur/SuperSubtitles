...
func TestConvertSuperSubtitle(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expected: models.Subtitle{
				...
				Release:       "Outlander (Season 1) (1080p)",
				ReleaseGroups: []string{"Outlander (Season 1) (1080p)"},
		...
		},
		{
			...
			expected: models.Subtitle{
				...
				Release:       "Outlander - 7x01 (AMZN.WEB-DL.720p-NTb)",
				ReleaseGroups: []string{"Outlander - 7x01 (AMZN.WEB-DL.720p-NTb)"},
		...
		},
		{
			...
			expected: models.Subtitle{
				...
				Release:       "Some Movie",
				ReleaseGroups: []string{"Some Movie"},
				Source:        "Some Movie",
				IsSeasonPack:  false,
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(result.ReleaseGroups) != len(tt.expected.ReleaseGroups) {
				t.Errorf("ReleaseGroups length: expected %d, got %d", len(tt.expected.ReleaseGroups), len(result.ReleaseGroups))
			}
		})
	}
}
